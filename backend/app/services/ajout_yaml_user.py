...
def get_user_response():
    ...
    while True:
        ...
        time.sleep(2)  # Attendre 1 seconde avant de réessayer
        
def send_validation(command, is_valid, reason):
    """Envoie une requête au backend pour informer qu'on attend une validation."""
    response = requests.post("http://127.0.0.1:5000/validation", json={'command': command, 'is_valid' : is_valid, 'reason' : reason})
    return response.json()
    ...
    **Important** :
    ...
    while not phase2:
        ...
        if user_confirmation == 'o':
            ...
            if user_alternative:
                ...
                #     f"reason: str (Explication concise de la validation ou de l'invalidation en 2 ou 3 phrases)\n"
                ...
                # send_validation(user_alternative, is_valid, reason)  # Envoie la validation au backend
    ...
    for step in range(1, 4):  # 9 étapes supplémentaires
        ...
        if user_confirmation == 'o':
            ...
            if user_alternative:
                print("rempalcement de la commande d'énumération par celle de l'utilisateur...")
                enumerate_command = user_alternative
                # # Étape 1 : Envoyer la commande alternative à Mistral pour validation
                # prompt_for_validation = (
                #     f"Contexte :\n"
